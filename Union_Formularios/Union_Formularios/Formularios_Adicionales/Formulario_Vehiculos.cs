...
namespace Formulario_Principal_Car_EFULL.Formularios
{
    public partial class Formulario_Vehiculos : Form
    {
        ...
        private bool _warnedInactivo = false;
        ...
        private void FormVehiculo_Load(object sender, EventArgs e)
        {
            ...
            _tt = new ToolTip { IsBalloon = true, AutoPopDelay = 6000, InitialDelay = 400, ReshowDelay = 200, ShowAlways = true };
            _ep = new ErrorProvider { BlinkStyle = ErrorBlinkStyle.NeverBlink };

            _tt.SetToolTip(txt_Placa, "Formato: ABC-1234 (mayúsculas y guion).");
            _tt.SetToolTip(txt_Num_Chasis, "VIN: 17 caracteres alfanuméricos.");
            _tt.SetToolTip(txt_Kilometraje, "Entero ≥ 0 (en KM).");
            _tt.SetToolTip(Cmbox_Estado, "Activo: operativo. Inactivo: archivado, no se puede facturar/planificar.");
            _tt.SetToolTip(btn_Seleccionar_Propietario, "Buscar y asignar un propietario registrado.");

            txt_Placa.Leave += txt_Placa_Leave_Format;  // auto-formato AAA-1234
            txt_Num_Chasis.TextChanged += AnyChanged_Validate;
            txt_Num_Motor.TextChanged += AnyChanged_Validate;
            txt_Color.TextChanged += AnyChanged_Validate;
            txt_Kilometraje.TextChanged += AnyChanged_Validate;
            Cmbox_Combustible.SelectedIndexChanged += AnyChanged_Validate;
            Cmbox_Estado.SelectedIndexChanged += Cmbox_Estado_SelectedIndexChanged;
            Cmbox_Tip_Vehic.SelectedIndexChanged += AnyChanged_Validate;
            Cmbox_Marca.SelectedIndexChanged += AnyChanged_Validate;
            Cmbox_Modelo.SelectedIndexChanged += AnyChanged_Validate;
            cmb_Año.SelectedIndexChanged += AnyChanged_Validate;

            btn_Guardar_Vehiculo.Enabled = false;
        }
        ...
        private void SetInitialState()
        {
            ...
            txt_Prop_Asig.Clear();
            txt_Prop_Asig.Tag = null;   // <-- importante
        ...
        private void Cmbox_Estado_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (_suspendEvents) return;
            if (Cmbox_Estado.Text == "Inactivo" && !_warnedInactivo)
            {
                _warnedInactivo = true;
                MessageBox.Show(
                    "Al marcar INACTIVO, el vehículo quedará archivado y no podrá facturarse ni planificarse hasta reactivarlo.",
                    "Estado: Inactivo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            ValidateForm();
        }
        ...
        private void btn_Guardar_Vehiculo_Click(object sender, EventArgs e)
        {
            try
            {
                // ---------- Normalización de PLACA: AAA-1234 ----------
                ...
                if (!Regex.IsMatch(placa, @"^[A-Z]{3}-\d{4}$"))
                {
                    MessageBox.Show("Placa inválida. Formato esperado: ABC-1234.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txt_Placa.Focus();
                    return;
                }

                // ---------- Recuperar/validar propietario ----------
                // Si el ID interno está vacío, intenta recuperarlo desde Tag
                if (idPropietarioSeleccionado <= 0 && txt_Prop_Asig.Tag is int tagId && tagId > 0)
                    idPropietarioSeleccionado = tagId;
                ...
                if (idPropietarioSeleccionado <= 0)
                {
                    ...
                    btn_Seleccionar_Propietario.Focus();
                    return;
                }

                // ---------- Validaciones de combos dependientes ----------
                if (Cmbox_Tip_Vehic.SelectedValue == null) { MessageBox.Show("Seleccione el tipo de vehículo.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); Cmbox_Tip_Vehic.Focus(); return; }
                if (Cmbox_Marca.SelectedValue == null) { MessageBox.Show("Seleccione la marca.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); Cmbox_Marca.Focus(); return; }
                if (Cmbox_Modelo.SelectedValue == null) { MessageBox.Show("Seleccione el modelo.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); Cmbox_Modelo.Focus(); return; }
                if (cmb_Año.SelectedValue == null) { MessageBox.Show("Seleccione el año del modelo.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); cmb_Año.Focus(); return; }

                // ---------- Validaciones de campos finales ----------
                string motor = (txt_Num_Motor.Text ?? "").Trim().ToUpper();
                if (string.IsNullOrWhiteSpace(motor)) { MessageBox.Show("Ingrese N° de Motor.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); txt_Num_Motor.Focus(); return; }

                string chasis = (txt_Num_Chasis.Text ?? "").Trim().ToUpper();
                if (!Regex.IsMatch(chasis, @"^[A-Z0-9]{17}$"))
                {
                    MessageBox.Show("El VIN (N° de Chasis) debe tener 17 caracteres alfanuméricos.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txt_Num_Chasis.Focus();
                    return;
                }

                string color = (txt_Color.Text ?? "").Trim();
                if (string.IsNullOrWhiteSpace(color)) { MessageBox.Show("Ingrese el color.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); txt_Color.Focus(); return; }

                string combus = (Cmbox_Combustible.Text ?? "").Trim();
                if (string.IsNullOrWhiteSpace(combus)) { MessageBox.Show("Seleccione el combustible.", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); Cmbox_Combustible.Focus(); return; }

                if (!int.TryParse((txt_Kilometraje.Text ?? "").Trim(), out int km) || km < 0)
                {
                    MessageBox.Show("Kilometraje inválido (entero ≥ 0).", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txt_Kilometraje.Focus();
                    return;
                }

                string estado = (Cmbox_Estado.Text ?? "").Trim();
                if (string.IsNullOrWhiteSpace(estado)) { MessageBox.Show("Seleccione el estado (Activo/Inactivo).", "Validación", MessageBoxButtons.OK, MessageBoxIcon.Warning); Cmbox_Estado.Focus(); return; }

                // ---------- IDs seleccionados ----------
                int tipoId = Convert.ToInt32(Cmbox_Tip_Vehic.SelectedValue);
                int marcaId = Convert.ToInt32(Cmbox_Marca.SelectedValue);
                int modeloId = Convert.ToInt32(Cmbox_Modelo.SelectedValue);
                int modeloAnioId = Convert.ToInt32(cmb_Año.SelectedValue);

                // ---------- Modo edición/alta ----------
                bool isEdit = _vehicleId.HasValue;
                int currentId = _vehicleId.GetValueOrDefault();
                ...
                using (var cn = Conexion_SQL.GetConnection())
                {
                    ...
                    if (isEdit)
                    {
                        using (var chk = new SqlCommand("SELECT COUNT(1) FROM dbo.Vehiculos WHERE Placa = @p AND VehicleID <> @id;", cn))
                        {
                            chk.Parameters.AddWithValue("@p", placa);
                            chk.Parameters.AddWithValue("@id", currentId);
                            if (Convert.ToInt32(chk.ExecuteScalar()) > 0)
                            {
                                MessageBox.Show("Ya existe otro vehículo con esa placa.", "Duplicado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
                    }
                    ...
                    {
                        using (var chk = new SqlCommand("SELECT COUNT(1) FROM dbo.Vehiculos WHERE Placa = @p;", cn))
                        {
                            chk.Parameters.AddWithValue("@p", placa);
                            if (Convert.ToInt32(chk.ExecuteScalar()) > 0)
                            {
                                MessageBox.Show("Ya existe un vehículo con esa placa.", "Duplicado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                return;
                            }
                        }
                    }
...
(@Placa, @TipoID, @MarcaID, @ModeloID, @ModeloAnioID, @Motor, @Chasis, @Color, @Combustible, @Kilometraje, @Estado, @Prop);";
                        ...
                        MessageBox.Show("Vehículo registrado correctamente.", "Vehículos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Limpiar_Campos();
                    }
                    else
                    {
...
WHERE VehicleID = @ID;";
                        ...
                        MessageBox.Show("Vehículo actualizado correctamente.", "Vehículos", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        Limpiar_Campos();
                    }
                }
            }
        ...
        private void CargarVehiculo(int id)
        {
            try
            {
                ...
                using (var cn = Conexion_SQL.GetConnection())
                {
                    ...
                    WHERE V.VehicleID = @id;", cn))
                    {
                        ...
                        {
                            if (!rd.Read()) { MessageBox.Show("No se encontró el vehículo."); return; }

                            _vehicleId = id;

                            txt_Placa.Text = (rd["Placa"]?.ToString() ?? "").ToUpper();
                            txt_Placa.ReadOnly = true; txt_Placa.Enabled = false;

                            tipoId = rd["TipoID"] as int?;
                            marcaId = rd["MarcaID"] as int?;
                            modeloId = rd["ModeloID"] as int?;
                            modeloAnioId = rd["ModeloAnioID"] as int?;
                ...
                // cascada de combos por IDs
                CargarTiposDesdeBD();
                if (tipoId.HasValue) Cmbox_Tip_Vehic.SelectedValue = tipoId.Value; else SelectByTextIfNeeded(Cmbox_Tip_Vehic, tipoN);
                Cmbox_Tip_Vehic.Enabled = true;

                if (tipoId.HasValue) CargarMarcasDesdeBD(tipoId.Value); else ResetCombo(Cmbox_Marca);
                if (marcaId.HasValue) Cmbox_Marca.SelectedValue = marcaId.Value; else SelectByTextIfNeeded(Cmbox_Marca, marcaN);
                Cmbox_Marca.Enabled = true;

                if (marcaId.HasValue) CargarModelosDesdeBD(marcaId.Value); else ResetCombo(Cmbox_Modelo);
                if (modeloId.HasValue) Cmbox_Modelo.SelectedValue = modeloId.Value; else SelectByTextIfNeeded(Cmbox_Modelo, modeloN);
                Cmbox_Modelo.Enabled = true;

                if (modeloId.HasValue) CargarAniosDesdeBD(modeloId.Value); else ResetCombo(cmb_Año);
                if (modeloAnioId.HasValue) cmb_Año.SelectedValue = modeloAnioId.Value; else SelectByTextIfNeeded(cmb_Año, anio == 0 ? "" : anio.ToString());
                cmb_Año.Enabled = true;
            ...
            {
                _suspendEvents = false;
                MessageBox.Show("Error al cargar vehículo: " + ex.Message, "Vehículos",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        // ===== 11) VALIDACIONES DE TECLADO =====

        private void AnyChanged_Validate(object sender, EventArgs e)
        {
            if (_suspendEvents) return;
            ValidateForm();
        }
        ...
        private bool ValidateForm()
        {
            ...
            bool placaOk = Regex.IsMatch(placa, @"^[A-Z]{3}-\d{4}$");
            if (!placaOk) { _ep.SetError(txt_Placa, "Placa inválida. Ej.: ABC-1234"); ok = false; }

            if (Cmbox_Tip_Vehic.SelectedValue == null) { _ep.SetError(Cmbox_Tip_Vehic, "Seleccione el tipo."); ok = false; }
            if (Cmbox_Marca.SelectedValue == null) { _ep.SetError(Cmbox_Marca, "Seleccione la marca."); ok = false; }
            if (Cmbox_Modelo.SelectedValue == null) { _ep.SetError(Cmbox_Modelo, "Seleccione el modelo."); ok = false; }
            if (cmb_Año.SelectedValue == null) { _ep.SetError(cmb_Año, "Seleccione el año."); ok = false; }

            if (idPropietarioSeleccionado <= 0) { _ep.SetError(btn_Seleccionar_Propietario, "Debe asignar un propietario."); ok = false; }

            if (string.IsNullOrWhiteSpace(txt_Num_Motor.Text)) { _ep.SetError(txt_Num_Motor, "Ingrese N° de motor."); ok = false; }

            string vin = (txt_Num_Chasis.Text ?? "").Trim().ToUpper();
            if (!Regex.IsMatch(vin, @"^[A-Z0-9]{17}$")) { _ep.SetError(txt_Num_Chasis, "VIN debe tener 17 caracteres alfanuméricos."); ok = false; }

            if (string.IsNullOrWhiteSpace(txt_Color.Text)) { _ep.SetError(txt_Color, "Ingrese el color."); ok = false; }

            if (string.IsNullOrWhiteSpace(Cmbox_Combustible.Text)) { _ep.SetError(Cmbox_Combustible, "Seleccione el combustible."); ok = false; }

            if (!int.TryParse((txt_Kilometraje.Text ?? "").Trim(), out int km) || km < 0)
            { _ep.SetError(txt_Kilometraje, "Kilometraje inválido (entero ≥ 0)."); ok = false; }

            if (string.IsNullOrWhiteSpace(Cmbox_Estado.Text)) { _ep.SetError(Cmbox_Estado, "Seleccione el estado."); ok = false; }

            btn_Guardar_Vehiculo.Enabled = ok;
            return ok;
        }
        ...
        private void txt_Placa_Leave_Format(object sender, EventArgs e)
        {
            var raw = (txt_Placa.Text ?? "").Trim().ToUpper().Replace(" ", "").Replace("_", "");
            if (Regex.IsMatch(raw, @"^[A-Z]{3}\d{4}$"))
                txt_Placa.Text = raw.Substring(0, 3) + "-" + raw.Substring(3);
        ...
        public void EstablecerPropietario(int id, string nombreCompleto)
        {
            ...
            txt_Prop_Asig.Tag = id; 

            _ep?.SetError(btn_Seleccionar_Propietario, "");
