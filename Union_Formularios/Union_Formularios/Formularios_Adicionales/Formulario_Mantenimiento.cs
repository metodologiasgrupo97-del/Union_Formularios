...
namespace Formulario_Principal_Car_EFULL.Formularios
{
    ...
    {
        // Claves del vehículo derivadas de la placa
        private int? _vehTipoID, _vehMarcaID, _vehModeloID;

        // Helpers de UI (money labels)
        private readonly Dictionary<Label, int> _rightEdgeMoney = new Dictionary<Label, int>();
        private readonly CultureInfo _ec = new CultureInfo("es-EC");

        // Fallback opcional si TipoServicio no trae precio
        private readonly Dictionary<string, decimal> preciosServicio = new Dictionary<string, decimal>
        {
            { "Cambio de aceite", 25 },
            { "Cambio de frenos", 40 },
            { "Alineación", 35 },
        ...
        // Controles (deben existir en el Designer)
        private Label label9;                       // Etiqueta "Tipo repuesto"
        private Guna2ComboBox Cmbox_Tipo_Repuesto; // Combo "Repuestos originales / genéricos"
        private Label label11;                      // Etiqueta "Categoría"
        private Guna2ComboBox Cmbox_Cat_Repuesto;  // Combo Categorías
        ...
        private void Formulario_Mantenimiento_Load(object sender, EventArgs e)
        {
            ...
            InicializarFechaMantenimiento();
            CargarIvaVigente();
            PrepareMoneyLabels(lb_valor_servicio, lbl_sub_total, lbl_iva, lbl_total_a_pagar);

            // 2) DataGridView base (sin data aún)
            ...
            // Eventos del grid
            dgvRepuestos.EditingControlShowing -= dgvRepuestos_EditingControlShowing;
            dgvRepuestos.EditingControlShowing += dgvRepuestos_EditingControlShowing;
            dgvRepuestos.CurrentCellDirtyStateChanged -= dgvRepuestos_CurrentCellDirtyStateChanged;
            dgvRepuestos.CurrentCellDirtyStateChanged += dgvRepuestos_CurrentCellDirtyStateChanged;
            dgvRepuestos.CellValidating -= dgvRepuestos_CellValidating;
            dgvRepuestos.CellValidating += dgvRepuestos_CellValidating;
            dgvRepuestos.DataError -= dgvRepuestos_DataError;
            dgvRepuestos.DataError += dgvRepuestos_DataError;
            dgvRepuestos.CellValueChanged -= dgvRepuestos_CellValueChanged;
            dgvRepuestos.CellValueChanged += dgvRepuestos_CellValueChanged;
            ...
            // 4) Combos de filtro (visibles sólo con rbtn_si_GM)
            Cmbox_Tipo_Repuesto.Items.Clear();
            Cmbox_Tipo_Repuesto.Items.AddRange(new object[] { "Repuestos originales", "Repuestos genéricos" });
            Cmbox_Tipo_Repuesto.SelectedIndex = -1;

            CargarCategoriasCombo();

            if (label9 != null) label9.Visible = false;      // "Tipo repuesto"
            if (label11 != null) label11.Visible = false;    // "Categoría"
            Cmbox_Tipo_Repuesto.Visible = false;
            Cmbox_Cat_Repuesto.Visible = false;

            // Eventos de filtro
            Cmbox_Tipo_Repuesto.SelectedIndexChanged -= (s, e2) => RefrescarGridRepuestosFiltrado();
            Cmbox_Tipo_Repuesto.SelectedIndexChanged += (s, e2) => RefrescarGridRepuestosFiltrado();
            Cmbox_Cat_Repuesto.SelectedIndexChanged -= (s, e2) => RefrescarGridRepuestosFiltrado();
            Cmbox_Cat_Repuesto.SelectedIndexChanged += (s, e2) => RefrescarGridRepuestosFiltrado();

            // 5) Placas (simple)
            using (var cn = Conexion_SQL.OpenConnection())
            using (var cmd = new SqlCommand("SELECT Placa FROM dbo.Vehiculos ORDER BY Placa;", cn))
            using (var r = cmd.ExecuteReader())
            {
                ...
                while (r.Read())
                {
                    ...
                    if (!string.IsNullOrEmpty(placa)) Cmbox_Select_Plaque.Items.Add(placa);
                }
            }
            ...
            // Habilitar "usar repuestos" solo si hay placa
            rbtn_si_GM.Enabled = Cmbox_Select_Plaque.SelectedIndex >= 0;
        }

        // ==== Trabajador seleccionado desde otro formulario
        public void EstablecerTrabajador(int id, string nombreCompleto)
        {
            _selectedUserId = id;
            txt_Show_Mecanico_Res.Text = nombreCompleto ?? string.Empty;
            txt_preview_mecanico.Text = nombreCompleto ?? string.Empty;
        }

        // ==== Servicios desde BD (Nombre + PrecioServicio)
        private void CargarServiciosDesdeBD()
        {
            try
            {
                ...
                using (var da = new SqlDataAdapter(
                    "SELECT TipoServicioID, Nombre, PrecioServicio FROM dbo.TipoServicio ORDER BY Nombre;", cn))
                {
                    var t = new DataTable();
                    da.Fill(t);
                    Cmbox_Tip_Service_Realizado.DataSource = t;
                    Cmbox_Tip_Service_Realizado.DisplayMember = "Nombre";
                    Cmbox_Tip_Service_Realizado.ValueMember = "TipoServicioID";
                    Cmbox_Tip_Service_Realizado.SelectedIndex = -1;
                }
            }
            catch
            {
                // fallback silencioso: deja la lista vacía si falla
                Cmbox_Tip_Service_Realizado.DataSource = null;
                Cmbox_Tip_Service_Realizado.Items.Clear();
            }
        }

        // Precio del servicio seleccionado (con fallback a diccionario)
        private decimal ObtenerPrecioServicioSeleccionado()
        {
            if (Cmbox_Tip_Service_Realizado.SelectedItem is DataRowView drv
                && drv.Row.Table.Columns.Contains("PrecioServicio"))
            {
                var v = drv["PrecioServicio"];
                if (v != null && v != DBNull.Value) return Convert.ToDecimal(v);
            }
            if (preciosServicio.TryGetValue(Cmbox_Tip_Service_Realizado.Text ?? "", out var val))
                return val;
            return 0m;
        }

        // ==== Carga categorías dinámicamente para el combo de filtro
        private void CargarCategoriasCombo()
        {
            try
            {
                using (var cn = Conexion_SQL.OpenConnection())
                using (var da = new SqlDataAdapter(
                    "SELECT DISTINCT Categoria FROM dbo.Repuestos WHERE Categoria IS NOT NULL ORDER BY Categoria;", cn))
                {
                    var t = new DataTable();
                    da.Fill(t);
                    ...
                    foreach (DataRow r in t.Rows)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(c)) Cmbox_Cat_Repuesto.Items.Add(c);
                    }
        ...
        // ==== Obtiene claves del vehículo por placa (TipoID/MarcaID/ModeloID)
        private bool TryGetVehiculoClaves(string placa)
        {
            _vehTipoID = _vehMarcaID = _vehModeloID = null;
            ...
            try
            {
                ...
                using (var cmd = new SqlCommand(
                    "SELECT TipoID, MarcaID, ModeloID FROM dbo.Vehiculos WHERE LTRIM(RTRIM(Placa)) = @p;", cn))
                {
                    cmd.Parameters.Add("@p", SqlDbType.NVarChar, 10).Value = placa.Trim();
                    using (var r = cmd.ExecuteReader())
                    {
                        if (!r.Read()) return false;
                        _vehTipoID = r.IsDBNull(0) ? (int?)null : r.GetInt32(0);
                        _vehMarcaID = r.IsDBNull(1) ? (int?)null : r.GetInt32(1);
                        _vehModeloID = r.IsDBNull(2) ? (int?)null : r.GetInt32(2);
                        return _vehTipoID.HasValue && _vehMarcaID.HasValue && _vehModeloID.HasValue;
                    }
                }
            }
            catch { return false; }
        ...
        // ==== Refresca grid con filtro (Originales x Marca+Modelo) + (Genéricos x Tipo)
        private void RefrescarGridRepuestosFiltrado()
        {
            ...
            // Determina claves del vehículo por placa
            if (!TryGetVehiculoClaves(Cmbox_Select_Plaque.Text))
            {
                dgvRepuestos.DataSource = null;
                dgvRepuestos.Columns.Clear();
                return;
            ...
            string filtroTipo = Cmbox_Tipo_Repuesto.SelectedItem?.ToString();   // "Repuestos originales" | "Repuestos genéricos" | null
            string filtroCat = Cmbox_Cat_Repuesto.SelectedItem?.ToString();     // categoría o null
...
SELECT
    r.RepuestoID, r.Codigo, r.Nombre, r.Categoria, r.TipoRepuesto,
    r.Marca, r.Modelo, r.PrecioUnitario, r.Stock
FROM dbo.Repuestos r
WHERE r.Activo = 1
  AND (
        (r.TipoRepuesto = N'Repuestos originales' AND r.MarcaID = @MarcaID AND r.ModeloID = @ModeloID)
     OR (r.TipoRepuesto = N'Repuestos genéricos'  AND r.TipoID  = @TipoID)
      )");

                    var cmd = new SqlCommand(); cmd.Connection = cn;
                    cmd.Parameters.AddWithValue("@TipoID", _vehTipoID.Value);
                    cmd.Parameters.AddWithValue("@MarcaID", _vehMarcaID.Value);
                    cmd.Parameters.AddWithValue("@ModeloID", _vehModeloID.Value);

                    if (!string.IsNullOrWhiteSpace(filtroTipo))
                    {
                        sb.Append(" AND r.TipoRepuesto = @TipoRep");
                        cmd.Parameters.AddWithValue("@TipoRep", filtroTipo);
                    }
                    if (!string.IsNullOrWhiteSpace(filtroCat))
                    {
                        sb.Append(" AND r.Categoria = @Cat");
                        cmd.Parameters.AddWithValue("@Cat", filtroCat);
                    }

                    sb.Append(" ORDER BY r.Nombre;");
            ...
            {
                MessageBox.Show("No se pudo aplicar el filtro de repuestos:\n" + ex.Message,
        ...
        private void InicializarFechaMantenimiento()
        {
            ...
            DTP_Date_Mantenimiento.Format = DateTimePickerFormat.Long;
            DTP_Date_Mantenimiento.ShowCheckBox = false;
            DTP_Date_Mantenimiento.MinDate = new DateTime(2000, 1, 1);
            DTP_Date_Mantenimiento.MaxDate = hoy;
            DTP_Date_Mantenimiento.Value = hoy;

            txt_Show_Date.Text = hoy.ToString("dd/MM/yyyy");
        }

        // ==== Agrega columnas "Cantidad" y "Seleccionar" si no existen
        ...
        private void dgvRepuestos_CellValidating(object sender, DataGridViewCellValidatingEventArgs e)
        {
            ...
            if (!int.TryParse(texto, out int cantidad) || cantidad < 1)
            {
                e.Cancel = true;
        ...
        {
            if (e.RowIndex < 0) return;
            var col = dgvRepuestos.Columns[e.ColumnIndex].Name;
        ...
        private void ActualizarListaRepuestos()
        {
            ...
            foreach (DataGridViewRow row in dgvRepuestos.Rows)
            {
                ...
                if (row.Cells["Seleccionar"].Value is bool sel && sel)
                {
        ...
        private static int GetInt(object v, int def = 0)
        {
            ...
            return int.TryParse(Convert.ToString(v), out int n) ? n : def;
        }
        ...
        private void btn_Grabar_Datos_factura_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Precio servicio desde BD (con fallback)
                decimal costoServicio = ObtenerPrecioServicioSeleccionado();
                ...
                foreach (DataGridViewRow row in dgvRepuestos.Rows)
                {
                    ...
                    if (!int.TryParse(Convert.ToString(row.Cells["RepuestoID"]?.Value), out int repuestoID)) continue;

                    if (!int.TryParse(Convert.ToString(row.Cells["Cantidad"]?.Value), out int cantidad) || cantidad < 1)
                        cantidad = 1;
                ...
                DateTime fechaMant = (DTP_Date_Mantenimiento.ShowCheckBox && !DTP_Date_Mantenimiento.Checked)
                    ? DateTime.Today
                    : DTP_Date_Mantenimiento.Value.Date;
                object tsId = DBNull.Value;
                if (Cmbox_Tip_Service_Realizado.SelectedIndex >= 0 && Cmbox_Tip_Service_Realizado.SelectedValue != null) tsId = Cmbox_Tip_Service_Realizado.SelectedValue;
...
INSERT INTO dbo.Facturas
 (CodigoFactura, Fecha, ID_Propietario, VehicleID, MetodoPago, FormaPago, Moneda,
  Subtotal, IVA, Total, FechaMantenimiento, TipoServicioID, UserID, Observaciones)
VALUES
...
SELECT CAST(SCOPE_IDENTITY() AS int);";
                        ...
                        using (var cmdFactura = new SqlCommand(insertFactura, cn, tx))
                        {
                            ...
                            cmdFactura.Parameters.AddWithValue("@TipoServicioID", tsId);
...
VALUES (@FacturaID, @RepuestoID, @Cantidad, @PrecioUnitario, @IVA);", cn, tx))
                        ...
                        SetMoney(lbl_sub_total, subtotal);
                        SetMoney(lbl_iva, iva);
                        SetMoney(lbl_total_a_pagar, totalFactura);
                        ...
                        SetMoney(lb_valor_servicio, costoServicio);
                        ActualizarListaRepuestos();
                    }
        ...
        private void btn_Guardar_Datos_factura_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                fecha = (DTP_Date_Mantenimiento.ShowCheckBox && !DTP_Date_Mantenimiento.Checked)
                    ? DateTime.Today
                    : DTP_Date_Mantenimiento.Value.Date;
            }
            ...
            // Precio desde BD (con fallback)
            decimal costoServicio = ObtenerPrecioServicioSeleccionado();
            SetMoney(lb_valor_servicio, costoServicio);

            var repuestosSeleccionados = new List<string>();
            decimal totalRepuestos = 0m;
        ...
        {
            var formTrabajador = new Formulario_Seleccionar_Trabajador(this);
        ...
        private void rbtn_si_GM_CheckedChanged(object sender, EventArgs e)
        {
            if (rbtn_si_GM.Checked)
            {
                ...
                // Mostrar combos de filtro
                if (label9 != null) label9.Visible = true;
                if (label11 != null) label11.Visible = true;
                Cmbox_Tipo_Repuesto.Visible = true;
                Cmbox_Cat_Repuesto.Visible = true;

                dgvRepuestos.Enabled = true;
                RefrescarGridRepuestosFiltrado();
            }
            else
            {
                // Ocultar combos de filtro
        ...
        private void Cmbox_Select_Plaque_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (rbtn_si_GM.Checked) RefrescarGridRepuestosFiltrado();
        }
        ...
        private void PrepareMoneyLabels(params Label[] labels)
        {
            foreach (var lbl in labels)
            {
                ...
                if (!_rightEdgeMoney.ContainsKey(lbl)) _rightEdgeMoney[lbl] = lbl.Right;
                lbl.SizeChanged -= MoneyLabel_SizeChanged;
                lbl.SizeChanged += MoneyLabel_SizeChanged;
                if (lbl.Parent != null)
                {
                    lbl.Parent.Resize -= MoneyLabel_ParentResize;
                    lbl.Parent.Resize += MoneyLabel_ParentResize;
                }
                FixRight(lbl);
            }
        }
        private void MoneyLabel_SizeChanged(object sender, EventArgs e)
        {
            var lbl = sender as Label;
            FixRight(lbl);
        }
        private void MoneyLabel_ParentResize(object sender, EventArgs e)
        {
            foreach (var kv in _rightEdgeMoney) FixRight(kv.Key);
        }
        private void FixRight(Label lbl)
        {
            if (lbl == null) return;
            if (_rightEdgeMoney.TryGetValue(lbl, out int right))
                lbl.Left = right - lbl.Width;
        }

        private string FormatMoneyCompact(decimal value)
        {
            string sign = value < 0 ? "-" : "";
            decimal abs = Math.Abs(value);
            if (abs >= 1000000m) return "$ " + (abs / 1000000m).ToString("0.#", _ec) + " M";
            if (abs >= 1000m) return "$ " + (abs / 1000m).ToString("0.#", _ec) + " K";
            return "$ " + abs.ToString("N2", _ec);
        }
        public void SetMoney(Label lbl, decimal value)
        {
            if (lbl == null) return;
            string t = FormatMoneyCompact(value);
            if (!_rightEdgeMoney.ContainsKey(lbl)) _rightEdgeMoney[lbl] = lbl.Right;
            lbl.Text = t;
            int w = TextRenderer.MeasureText(t, lbl.Font).Width + 6;
            lbl.Width = w;
            FixRight(lbl);
        }
        ...
        private void InitializeComponent()
        {
            ...
            this.Cmbox_Cat_Repuesto = new Guna.UI2.WinForms.Guna2ComboBox();
            this.label9 = new System.Windows.Forms.Label();
            this.Cmbox_Tipo_Repuesto = new Guna.UI2.WinForms.Guna2ComboBox();
            ...
            this.Cmbox_Select_Plaque.IntegralHeight = false;
            ...
            this.guna2ShadowPanel1.Controls.Add(this.Cmbox_Cat_Repuesto);
            this.guna2ShadowPanel1.Controls.Add(this.label9);
            this.guna2ShadowPanel1.Controls.Add(this.Cmbox_Tipo_Repuesto);
            ...
            this.label11.Font = new System.Drawing.Font("Montserrat SemiBold", 9.749999F, System.Drawing.FontStyle.Bold);
            this.label11.Location = new System.Drawing.Point(460, 216);
            this.label11.Name = "label11";
            this.label11.Size = new System.Drawing.Size(159, 20);
            this.label11.TabIndex = 91;
            this.label11.Text = "Categoria de repuesto";
            // 
            // Cmbox_Cat_Repuesto
            // 
            this.Cmbox_Cat_Repuesto.BackColor = System.Drawing.Color.Transparent;
            this.Cmbox_Cat_Repuesto.DrawMode = System.Windows.Forms.DrawMode.OwnerDrawFixed;
            this.Cmbox_Cat_Repuesto.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
            this.Cmbox_Cat_Repuesto.FocusedColor = System.Drawing.Color.FromArgb(((int)(((byte)(94)))), ((int)(((byte)(148)))), ((int)(((byte)(255)))));
            this.Cmbox_Cat_Repuesto.FocusedState.BorderColor = System.Drawing.Color.FromArgb(((int)(((byte)(94)))), ((int)(((byte)(148)))), ((int)(((byte)(255)))));
            this.Cmbox_Cat_Repuesto.Font = new System.Drawing.Font("Segoe UI", 12F);
            this.Cmbox_Cat_Repuesto.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(68)))), ((int)(((byte)(88)))), ((int)(((byte)(112)))));
            this.Cmbox_Cat_Repuesto.IntegralHeight = false;
            this.Cmbox_Cat_Repuesto.ItemHeight = 30;
            this.Cmbox_Cat_Repuesto.Location = new System.Drawing.Point(459, 240);
            this.Cmbox_Cat_Repuesto.Name = "Cmbox_Cat_Repuesto";
            this.Cmbox_Cat_Repuesto.Size = new System.Drawing.Size(276, 36);
            this.Cmbox_Cat_Repuesto.TabIndex = 92;
            // 
            // label9
            // 
            this.label9.AutoSize = true;
            this.label9.Font = new System.Drawing.Font("Montserrat SemiBold", 9.749999F, System.Drawing.FontStyle.Bold);
            this.label9.Location = new System.Drawing.Point(195, 216);
            this.label9.Name = "label9";
            this.label9.Size = new System.Drawing.Size(123, 20);
            this.label9.TabIndex = 89;
            this.label9.Text = "Tipo de repuesto";
            // 
            // Cmbox_Tipo_Repuesto
            // 
            this.Cmbox_Tipo_Repuesto.BackColor = System.Drawing.Color.Transparent;
            this.Cmbox_Tipo_Repuesto.DrawMode = System.Windows.Forms.DrawMode.OwnerDrawFixed;
            this.Cmbox_Tipo_Repuesto.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList;
            this.Cmbox_Tipo_Repuesto.FocusedColor = System.Drawing.Color.FromArgb(((int)(((byte)(94)))), ((int)(((byte)(148)))), ((int)(((byte)(255)))));
            this.Cmbox_Tipo_Repuesto.FocusedState.BorderColor = System.Drawing.Color.FromArgb(((int)(((byte)(94)))), ((int)(((byte)(148)))), ((int)(((byte)(255)))));
            this.Cmbox_Tipo_Repuesto.Font = new System.Drawing.Font("Segoe UI", 12F);
            this.Cmbox_Tipo_Repuesto.ForeColor = System.Drawing.Color.FromArgb(((int)(((byte)(68)))), ((int)(((byte)(88)))), ((int)(((byte)(112)))));
            this.Cmbox_Tipo_Repuesto.IntegralHeight = false;
            this.Cmbox_Tipo_Repuesto.ItemHeight = 30;
            this.Cmbox_Tipo_Repuesto.Location = new System.Drawing.Point(194, 240);
            this.Cmbox_Tipo_Repuesto.Name = "Cmbox_Tipo_Repuesto";
            this.Cmbox_Tipo_Repuesto.Size = new System.Drawing.Size(260, 36);
            this.Cmbox_Tipo_Repuesto.TabIndex = 90;
            // 
