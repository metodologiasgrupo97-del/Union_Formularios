...
namespace Union_Formularios.Formularios
{
    public partial class Formulario_Configuracion : Form
    {
        ...
        // ====== Perfil ======
        private int _userId = 0;                 // 0 => toma el primer Administrador
        private byte[] _fotoBytes = null;        // Foto actual o nueva
        ...
        {
            // Cargar perfil al mostrar el formulario
        ...
        private void CargarPerfil()
        {
            using (SqlConnection cn = new ConexionSQL_Implementacion().AbrirConexion())
            {
                ...
                // Si no hay userId explícito, toma el primer Administrador
                if (_userId == 0)
                {
                ...
                using (var cmd = new SqlCommand(sql, cn))
                {
                    ...
                    using (var rd = cmd.ExecuteReader())
                    {
                        ...
                        txt_Edit_Pass_Profile.Clear(); // pedir confirmación al guardar

                        // Foto de perfil
                        if (rd["FotoPerfil"] != DBNull.Value)
                        {
                            var buffer = (byte[])rd["FotoPerfil"];
                            _fotoBytes = buffer; // conserva por si no la cambian
                            var img = BytesToImage(buffer);
                            img_Perfil.Image = img; // pinta la imagen
                        }
                        else
                        {
                            _fotoBytes = null;
                            img_Perfil.Image = null; // o tu placeholder
                        }
                    }
                }
            }
        ...
        private void btn_Cargar_Foto_Perfil_Click(object sender, EventArgs e)
        {
            ...
            })
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    img_Perfil.Image = img;          // se ve al instante
                    _fotoBytes = ImageToBytes(img);   // listo para guardar
                }
            }
        }
        ...
        private void btn_Guardar_Act_Perfil_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Evitar duplicar LoginName en otros usuarios
        ...
        private static Image BytesToImage(byte[] bytes)
        {
            ...
            {
                // Clonar evita depender del stream (si no, a veces no se pinta)
